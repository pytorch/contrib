...
parser.add_argument('--swa_start', type=int, default=161, metavar='N', help='SWA start epoch number (default: 161)')
parser.add_argument('--swa_c', type=int, default=20, metavar='N', help='fast-SWA LR schedule cycle length (default: 20)')
...
def schedule(epoch):
    max_step = args.epochs + argc.swa_c
    t = epoch
    if t >= max_step:
        t = max_step - args.c +  (t - max_step) % args.c
    return (args.lr_init) * (1 + math.cos(math.pi * t / max_step)) / 2
